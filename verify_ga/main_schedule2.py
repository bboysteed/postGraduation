...
from schedule2_sko.GA import GA_TSP
...
target = Target(num_points_=23,exe_path_=os.path.join(os.path.abspath(os.path.dirname(__file__)),"schedule2","source.alt"),target_name_="schedule2")   
...
def main():    
    ...
    ga_tsp = GA_TSP(func=get_conv_rate, n_dim=target.num_points, crtp=createPopulation, size_pop=6, max_iter=14, prob_mut=0.5)
    visited_state_addr = []
