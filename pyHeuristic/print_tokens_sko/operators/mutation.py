...
def mut(individual):
    idxs = np.random.randint(
        0, individual.shape[0]-1, int(individual.shape[0]/10))
    for idx in idxs:
        if idx < 10:
...
def mut_schedule(individual):
    idxs = np.random.randint(
        0, individual.shape[0]-1, int(individual.shape[0]/2))
    for idx in idxs:
...
def mutation_replace(self):
    for i in range(self.size_pop):
        if np.random.rand() < self.prob_mut:
            self.Chrom[i] = mut_replace(self.Chrom[i])
    return self.Chrom
